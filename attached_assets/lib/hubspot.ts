...
export async function findContactByEmail(email: string) {
  const body = { filterGroups: [{ filters: [{ propertyName: 'email', operator: 'EQ', value: email }]}], properties: ['email','firstname','lastname'] }
...
export async function updateDealStage(dealId: string, pipeline?: string, dealstage?: string) {
  return hs(`/crm/v3/objects/deals/${dealId}`, 'PATCH', { properties: { pipeline, dealstage } })
}

export async function createNoteOnContact(contactId: string, bodyText: string) {
  const note = await hs('/crm/v3/objects/notes', 'POST', { properties: { hs_note_body: bodyText, hs_timestamp: new Date().toISOString() } })
  await hs(`/crm/v4/objects/notes/${note.id}/associations/contacts/${contactId}`, 'PUT', { types: [{ associationCategory:'HUBSPOT_DEFINED', associationTypeId: 202 }] })
  return note
}
